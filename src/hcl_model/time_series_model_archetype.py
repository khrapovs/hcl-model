...
class TimeSeriesModelArchetype(metaclass=abc.ABCMeta):
    ...
    def predict(self, num_steps: int, endog: pd.Series = None, exog: pd.DataFrame = None,
                quantile_levels: List[float] = None, **kwargs) -> pd.DataFrame:
        """
        Forecast the values and prediction intervals

        Parameters
        ----------
        num_steps: number of point in the future that we want to forecast
        endog: endogenous variables, if not provided the model should use the data provided into the fit() method
        exog: exogenous variables should cover the whole prediction horizon
        quantile_levels: list of desired prediction interval levels between 0 and 100 (in percentages).
            If not provided, no confidence interval will be given as output

        Returns
        -------
        A DataFrame containing values and prediction intervals.
    ...
    def simulate(self, num_steps: int, num_simulations: int, endog: pd.Series = None, exog: pd.DataFrame = None,
        ...
        Parameters
        ----------
        num_steps: number of points in the future that we want to simulate
        ...
        Returns
        -------
        A DataFrame containing simulations
    ...
    def _get_fitted_values(self) -> pd.Series:
        ...
        Returns
        -------
        One point ahead forecasts on the in-sample period which are the "fitted values" in time series context.
        """
    ...
    def _get_residuals(self) -> pd.Series:
        ...
        Returns
        -------
        Residuals of one point ahead forecasts on the in-sample period.
        """
    ...
    def _get_residual_moment(self, degree: int = 1, center_first: bool = False) -> float:
        """
        (Centered) moments of model fit residuals. Note that residuals can be biased on average.
        """
        if center_first:
    ...
    def _compute_prediction_quantiles(self, num_steps: int, num_simulations: int,
                                      quantile_levels: List[float] = None) -> pd.DataFrame:
    ...
    def _check_exogenous(exog: pd.DataFrame, nobs: int, num_steps: int):
        ...
        num_steps: the number of steps in the future we want to make forecast of
    ...
    def _get_in_sample_exog(self, endog: pd.Series) -> pd.DataFrame:
    ...
    def _get_out_sample_exog(self, num_steps: int = None) -> pd.DataFrame:
    ...
    @staticmethod
    def get_quantile_names(quantile_levels: List[float]):
