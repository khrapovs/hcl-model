...
class SARIMAXModel(TimeSeriesModelArchetype):
    ...
    def fit(self, endog: pd.Series, exog: pd.DataFrame = None, **kwargs):
        ...
        self._fit_results = sm.tsa.SARIMAX(self._endog, exog=self._get_in_sample_exog(self._endog),
                                           order=self._order,
                                           seasonal_order=self._seasonal_order,
                                           enforce_stationarity=self._enforce_stationarity).fit(disp=False)
    ...
    def simulate(self, num_steps: int, num_simulations: int, endog: pd.Series = None, exog: pd.DataFrame = None,
        ...
        sim_model = sm.tsa.SARIMAX(pd.Series(index=self._exog.iloc[idx].index),
    ...
    def _remove_trend(self, endog: pd.Series) -> pd.Series:
        if self._trend == 'n':
            ...
            name = endog.name
            trend = sm.tsa.tsatools.add_trend(self._endog, trend=self._trend, prepend=False)
    ...
    def _add_trend(self, df: pd.DataFrame, nobs: int) -> pd.DataFrame:
        if self._trend == 'n':
            ...
            exog = sm.tsa.tsatools.add_trend(pd.Series(np.ones(nobs + df.shape[0])),
                                             trend=self._trend, prepend=False, has_constant='add')
