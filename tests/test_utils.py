...
from hcl_model.utils import _get_duplicate_columns, decayed_weights, construct_calendar_exogenous
...
class TestUtils:
    ...
    def test_decayed_weights(self):
        for nobs in [5, 10, 99, 100, 101]:
            endog = pd.Series(np.random.normal(size=nobs),
                              index=pd.date_range('2018-01-01', periods=nobs, freq='W-FRI'))

            for weights in [decayed_weights(endog=endog),
...
class TestConstructCalendarExogenous:
    ...
    def test_construct_calendar_exogenous(self):
        ...
        holidays = [{self.lbl_holiday_name: self.lbl_cny,
