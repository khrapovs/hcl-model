...
class TestCalendarTransformer:
    ...
    def test_add_periodic_splines(self):
        ...
        df_default = cal_transformer.add_periodic_splines(
            df=xreg, degrees_of_freedom=degrees_of_freedom
        )
        ...
        df = cal_transformer.add_periodic_splines(
            df=xreg, degrees_of_freedom=degrees_of_freedom, input_level="isoweek"
        )
        ...
        df = cal_transformer.add_periodic_splines(
            df=xreg, degrees_of_freedom=degrees_of_freedom, input_level="month"
        )
        ...
        with pytest.raises(ValueError):
            cal_transformer.add_periodic_splines(
                df=xreg, degrees_of_freedom=degrees_of_freedom, input_level="day"
            )
    ...
    def test_add_holiday_triangles(self):
        ...
        for holiday, country in tasks:
            ...
            df2 = cal_transformer.add_holiday_triangles(
                xreg, holiday, country, merge_factors=True
            )

            assert df1.shape == (xreg.shape[0], xreg.shape[1] + 2)
            assert df2.shape == (xreg.shape[0], xreg.shape[1] + 1)

            assert set(df1.iloc[:, -2:].values.ravel()) <= set(
                [0.0] + lh_factors + rh_factors
            )
    ...
    def test_add_holiday_dummies(self):
        ...
        xreg_cny = (
            cal_transformer.add_holiday_dummies(
                xreg, self.lbl_cny, self.lbl_country_code_tw, lags=[1]
            ),
            [1],
        )
        xreg_ny = (
            cal_transformer.add_holiday_dummies(
                xreg, self.lbl_new_year, self.lbl_country_code_de, lags=[-1, 0, 2]
            ),
            [-1, 0, 2],
        )
        xreg_cc = (
        ...
        xreg_cgw = (
            cal_transformer.add_holiday_dummies(
                xreg, self.lbl_cgw_tw, self.lbl_country_code_tw
            ),
            None,
        )
...
class TestAddAutomaticSeasonalDummies:
    def test_add_automatic_seasonal_dummies(self):
        ...
        df = pd.DataFrame(
            ...
            index=pd.DatetimeIndex(
                pd.date_range("2017-01-01", periods=nobs, freq="W-FRI"), name=lbl_date
            ),
        )
        ...
        pd.testing.assert_frame_equal(
            df.sort_index(axis=1), df_result.sort_index(axis=1)
        )
    ...
    def test_add_automatic_seasonal_dummies_raises(self):
        ...
        df = pd.DataFrame(
            ...
            index=pd.DatetimeIndex(
                pd.date_range("2017-01-01", periods=nobs, freq=freq), name=lbl_date
            ),
        )
